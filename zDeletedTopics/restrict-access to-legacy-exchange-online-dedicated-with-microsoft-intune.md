...
---
# Restrict access to legacy Exchange Online Dedicated environment with Microsoft Intune
...
This topic is about how you can restrict to your  Exchange Online Dedicated environment in the legacy configuration. The workflow for this environment is similar to configuring conditional access for Exchange On-premises.

If you have an Exchange Online Dedicated environment in the new configuration, go to [new Exchange Online Dedicated](restrict-access-to-new-exchange-online-dedicated-with-microsoft-intune.md) article.
...
-   You must use the **on-premises Exchange connector** which connects [!INCLUDE[wit_nextref](../includes/wit_nextref_md.md)] to Microsoft Exchange On-premises. This lets you manage devices through the [!INCLUDE[wit_nextref](../includes/wit_nextref_md.md)] console. For details on the connector, see [Intune Exchange On-premises conector](intune-on-premises-exchange-connector.md).
    ...
    -   Make sure that you are using the latest version of the **on-premises Exchange connector**. The on-premise Exchange connector available to you in the Intune console is specific to your Intune tenant and cannot be used with any other tenant. You should also ensure that the exchange connector for your tenant is installed on exactly one machine and not on multiple machines.
        ...
        This connector should be downloaded from the Intune admin console.  For a walkthrough on how to configure the on-premises Exchange connector, see [Configure Microsoft Intune on-premises connector for on-premises or hosted Exchange](intune-on-premises-exchange-connector.md).
...
>If you intention is to first block all devices before granting access to email, checking the Block access, or Quarantine rule can be useful. The default rule will apply to all device types, so device types you configure as platform exceptions and that are unsupported by [!INCLUDE[wit_nextref](../includes/wit_nextref_md.md)] are also affected.
  ...
  >
  > For details, see [Configure Microsoft Intune Exchange on-premises connector](intune-on-premises-exchange-connector.md).
