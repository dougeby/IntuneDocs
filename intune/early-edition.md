...
IT admins can set app-based conditional policies via the new conditional access policy UI in Azure AD workload. The app-based conditional access policies that are in the Intune App Protection section in Azure will remain there for the time being and will be enforced side-by-side. There will also be a convenience link to the new conditional access policy UI in Azure AD from the Intune workload.
...
We are adding **Ownership Type** to the Device Details screen on the Company Portal app for iOS. This will allow users to find out more about privacy directly from this page from the Intune end user docs. They will also be able to locate this information on the About screen.
