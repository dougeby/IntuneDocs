...
---
# Setup Intune certificate connector for Symantec PKI Manager Web Service

[!INCLUDE[azure_portal](./includes/azure_portal.md)]

This article show you how to install and configure the Intune Certificate Connector to issue PKCS Certificates from a Symantec PKI Manager Web Service to Intune managed devices.

The Symantec PKI Manager Web Service is referred as Symantec CA throughout this article. If you already configured the Intune Certificate Connector to issue PKCS Certificates and SCEP Certificates from Microsoft Certification Authority (CA), the same Connector can be used to configure and issue PKCS Certificates from a Symantec CA. In this case, the Intune Certificate Connector can issue the following cetificates:

* PKCS Certificates from a Microsoft CA
* SCEP Certificates from a Microsoft CA
* PKCS Certificates from a Symantec CA

If you want to use Intune Certificate Connector for Microsoft CA and Symantec CA, you must complete the Intune Certificate Connector confuration for the Microsoft CA first then follow these steps to configure it for the Symantec CA.  For more details about configuring the Intune Certificate Connector for a Microsoft CA, see [Configure and manage PKCS Certificates with Intune](certificates-scep-configure.md).

## Prepare to install the Intune certificate connector

If you are already using the Intune Certificate Connector for an existing Microsoft CA, and you want to add Symantec CA support, skip this step and continue the remaining steps after you install the latest Intune Certificate Connector from the Intune Admin portal. This step is required only when you want to use the Intune Certificate Connector for a Symantec CA standalone.
...
4. Logon to the Symantec CA and navigate to **Get an RA Cert** from the tasks.
   ...
   - Provide the CSR content from Step 3 in the designated text box.
   - Provide the Certificate Friendly Name in the designated text box.
   - Click **Continue**.
      - This shows you a downloadable link for the RA Certificate.
      - Download the RA certificate to the local computer.

5. Import the RA Certificate into the Windows Certificate store:
   - Open a an MMC console.
      - Click **File** > **Add or Remove Snap-ins** > **Certificate** > then click **Add**.
   ...
   - Expand **Certificates (Local Computer)** > **Personal** > **Certificates**.
      - Right-click the **Certificates** node and select **All Tasks** > **Import**.
         - Select the location of the RA Certificate that downloaded from Symantec CA and click Next
         - Select **Personal Certificate Store** and click then **Next**.
         - Click **Finish**. This imports the RA Certificate into the Local Machine-Personal store along with its private key.  

6. Export and import the private key certificate
   - Expand **Certificates (Local Machine)** > **Personal** > **Certificates**.
   - Select the certificate that was imported in the previous step.
      - Right-click the certificate and choose **All Tasks** > **Export**.
         - Click **Next** and type the password.
         - Select the location to export to and click **Finish**.
   - Follow the same steps in Step 5 to import the private key certificate into the Local Computer-Personal store.
...
If you are already using the latest Intune Certificate Connector for an existing Microsoft CA and want to add Symantec CA support, skip this step. Otherwise, download the latest Intune Certificate Connector from the Intune admininstration portal and follow these instructions.
   ...
   > [!IMPORTANT]
   > If you want to configure the Intune Certificate Connector to issue certificates from a Microsoft CA and a Symantec CA, select **SCEP and PFX Profile Distribution**. Complete the remaining setup with the default selections.

   ![InstallConnector][InstallConnector]
 
## Configure the Intune Certificate Connector

The Intune Certificate Connector is installed at `%ProgramFiles%\Microsoft Intune` by default.
...
2. Open services.msc.
   - Select **Intune Connector Service**.
      - Stop the service and then start the service.
    - Close the Services window.

## Setup the Intune administrator account

If you are already using the Intune Certificate Connector for an existing Microsoft CA and want to add Symantec CA support, skip this step and continue with the remaining steps. 

1. Start the NDES Connector user interface from ` %ProgramFiles%\Microsoft Intune\NDESConnectorUI\NDESConnectorUI.exe `
   - Click the **Enrollment** tab and then click **Sign In**.
   - Provide your Intune tenant admin credentials in the designated text boxes.
   - Click **Sign In**.  It will display a confirmation dialog with a **Successfully enrolled** message as shown in the following screen shot.
...
1. Get a Trusted Root Certificate from the Symantec CA
   ...
   - Select the appropriate CA from CAs list.
   - Click Download root certificate to download the Trusted Root Certificate.
2. Create a Trusted Certificate Profile in the Intune administration portal.
   ...
   c. Provide the required information in **Name** and **Description** fields then select the target platform. 
      - Select the Trusted certificate profile from the Profile type drop down list.
      - Upload the Trusted Root Certificate that you obtained from Symantec CA in the previous step.
      - Complete the remaining steps in the profile creation. 
      - Click **Assignments** and select the appropriate group.  At least one user or device must be part of the assigned group.
...
2. Create a PKCS Certificate profile from **Microsoft Intune** > **Device Configuration – Profiles** > **Create profile** > **PKCS Certificate**.
   - Provide the required information in the **Name** and **Description** fields then select the target platform.
   - Select the **PKCS certificate profile** from the **Profile type** drop down list.  
   - Complete the remaining steps for the profile creation.
   ...
   > The following parameters of the PKCS Certificate Profile must be configured with specified values in the following table as shown in screen shot to issue PKCS Certificates through Intune Certificate Connector from Symantec CA. 
    ...
    | --- | --- | --- |
    | Certificate authority | pki-ws.symauth.com | This value must be Symantec CA base service URL without trailing slashes.  If you are not sure whether this is the correct base service URL for your Symantec CA subscription, contact Symantec Customer support. <br><br> If this URL is incorrect, the Intune Certificate Connector won't issue PKCS Certificates from the Symantec CA.| 
    | Certificate authority name | Symantec | This value must be the string **Symantec**. <br><br> If there is any change to this value, the Intune Certificate Connector won't issue PKCS Certificates from the Symantec CA.|
    | Certificate template name | Certificate Profile OID from Symantec CA. <br><br> Ex: `2.16.840.1.113733.1.16.1.2.3.1.1.61904612`| This value must be a Certificate Profile OID obtained in the previous section from the Symantec CA Certificate Profile template. <br><br> If the Intune Certificate Connector cannot find a certificate template associated with this Certificate Profile OID in the Symantec CA, it won't issue PKCS certificates from the Symantec CA.|
...
After completing the previous steps, the Intune Certificate Connector will issue PKCS Certificates from the Symantec CA to Intune managed devices in the assigned group. These certificates will be available in the Personal store of the Current User certificate store on the Intune managed device.
...
Use the information provided in this article in addition to that found in the article [What are Microsoft Intune device profiles?](device-profiles.md) to manage your organization's devices and the certificates on them.
