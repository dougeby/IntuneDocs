...
| The app failed to install. (0xC7D14FB5) | This error happens to Android LOB apps that are larger than 10 MB. The error is often associated with `ClientDisconnectException` (HTTP status code 499) which indicates a client side connection issue. Check to ensure the device has a good network connection. |
...
| The app installation was cancelled because the process was restarted during installation. (0xc7d14fbb) | The devices was rebooted during the app install so the install was cancelled. For available app, the user needs to install again from IW Portal; for required app it will be retried by the service. |
...
| The download failed because of an unknown error. (0xC7D14FB2) | This is a generic error when the download fails for none KNOX devices. This tends to be caused by WIFI issue or slow connection. |
...
| The download failed because of an unknown error. The policy will be retried the next time the device syncs. (0xC7D15078) | This error only happens to Android LOB applications. The failure could be related to the app size and limited network bandwidth. Try a location with good network connection and do a manual sync. |
...
| The app installation has failed. (0x87D13B64) | This error only happens to iOS store and LOB apps. We don't have additional details regarding this failure. |
