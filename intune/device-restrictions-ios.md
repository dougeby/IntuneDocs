...
- **Messages app on the device (supervised only)**: **Block** prevents users from using the Messages app for iMessage. If the device supports text messaging, the user can still send and receive text messages via SMS **Not configured** allows using the messages app to send and read messages over the internet.
