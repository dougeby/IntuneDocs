...
4. On the **Select groups** blade, choose the Azure AD groups which you want to include in, or exclude from this assignment. You can hold down the **CTRL** key to select multiple groups.
...
In case where mixed groups are used, or there are other conflicts, inclusion will take precedence over exclusion.
...
In this case, all your users and their devices will get the policy, even if the user’s device is part of the **All Devices** group. 

This is because exclusion only evaluates the direct members of the groups, and does not include devices that are associated with a user. However, devices that don't have a user will not get the policy because they will have no association to the **All Users** group. 

If you include **All Devices**, but exclude **All Users**, all the devices will receive the policy. The intent in this case is to exclude devices that have an associated user from this policy. However, it does not because the exclusion feature only compares direct group members. 

>[!Tip]
>Exclusions are not currently available for compliance policies or app assignment. 
>To exclude members from an assignment, you can use the Available, and Not applicable assignment intents. For example, you assign an app to **All corporate users** with the **Available** intent, and to **Senior Management Staff** with the **Not applicable** intent. the app is assigned to all users *except* those in the **Senior Management Staff** group. However, if you assign the app to **All corporate users** with the **Required** intent, the users in the **Senior Management Staff** group will not be excluded.
