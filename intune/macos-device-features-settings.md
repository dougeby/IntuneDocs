...
- Add many app ID-to-domain associations.
- Associate many domains with the same app ID.
...
- **App ID**: Enter the app identifier to associate with a website. The app identifier includes the team ID and a bundle ID: `TeamID.BundleID`.
...
> [!TIP]
> To troubleshoot, on your macOS device, open **System Preferences** > **Profiles**. Confirm the profile you created is the device profiles list. If it's listed, be sure the **Associated Domains Configuration** is in the profile, and it includes the correct app ID and domains.
