...
|**Shared devices**|Configure a managed Samsung KNOX Standard device as shared. In this mode, end users can sign in and out of the device with their Azure AD credentials. The device reamins managed whether it’s in use or not.<br>When end-users sign-in, they have access to apps and additionally get any policies applied to them. When users sign out, all app data is cleared.|No|Yes|
