...
The Intune SDK team will require your app's Application ID (Client ID). This can be found through the [AzurePortal](https://portal.azure.com/), under **All Applications**, in the column for **Application ID**. The Intune SDK team can be reached at emailing msintuneappsdk@microsoft.com. Also see the requirements for [Conditional Access](#ConditionalAccess) below.
...
1. Configure ADAL using the steps defined in the [Intune SDK for Android guide](#configure-azure-active-directory-authentication-library-adal).
    * To enable SSO, use [Common ADAL configurations](#common-adal-configurations), item 2 (App integrates ADAL).
> [!NOTE] 
