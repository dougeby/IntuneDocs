...
- **NDES Server**: A Microsoft Network Device Enrollment Service (NDES) server role running on Windows Server 2012 R2 or later.  NDES must not be installed to the same server that hosts the Enterprise CA, see [Network Device Enrollment Service Guidance](http://technet.microsoft.com/library/hh831498.aspx) for instructions on how to configure Windows Server 2012 R2 to host NDES.
...
- **Microsoft Intune Certificate Connector**: The **Certificate Connector** installer (**NDESConnectorSetup.exe**) can be downloaded from the Intune administration portal ready for installation on the NDES server.  

  - The NDES Certificate connector supports Federal Information Processing Standard (FIPS) mode when required, allowing certificate issuance and revocation when enabled.
...
If not using a reverse proxy such as WAP or Azure AD App Proxy, allow TCP traffic on port 443 from all hosts/IP addresses on the internet to the NDES server.

Allow all ports and protocols necessary between the NDES server and any supporting infrastructure.  e.g. The NDES server needs to communicate with the CA, DNS servers, Configuration Manager servers, domain controllers, and possibly other services within your environment.
...
|**Server authentication certificate**|Requested from your issuing CA or public CA; you install and bind this SSL certificate in IIS on the NDES server.  Note that a single certificate can be used as long as it has both client and server authentication key usages set within the certificates 'Enhanced Key Usages' attribute|
...
|Name|Details|
|--------|-----------|
|**NDES service account**|Enter a domain user account to use as the NDES Service account.  Note that the same service account could be used by the NDES server to support connection to Intune using the Intune Certificate Connector, and to communicate with the Enterprise CA when forwarding certificate requests.  ALternatively, seperate service accounts may be used for the Intune Certificate Connector and NDES to CA communication. |
...
1. On the server that hosts NDES, sign in as an **Enterprise Administrator**, and then use the [Add Roles and Features Wizard](https://docs.microsoft.com/previous-versions/windows/it-pro/windows-server-2012-R2-and-2012/hh831809(v=ws.11)) to install NDES:
   ...
   8. On the server, add the NDES service account as a member of the local **IIS_IUSR** group.
...
1. On your NDES Server, request and install a **server authentication** certificate from your internal CA or public CA. You then bind this SSL certificate in IIS.
    ...
    > [!TIP]
    > After you bind the SSL certificate in IIS, install a client authentication certificate. This certificate can be issued by any CA that is trusted by the NDES Server. You can use the same certificate for both server and client authentication as long as the certificate has both Enhance Key Usages (EKUs). Review the following steps for information about these authentication certificates.
...
8. In the **Certificate Connector** UI:
    ...
    Select **Sign In**, and enter your Intune service administrator credentials, or credentials for a tenant administrator with the global administration permission.  Note that the account credentials provided will be used continuously by the Intune Certificate Connector, not just to establish the connection to Intune.  It is highly recommended that you use a dedicated service account for NDES.
