...
#### Enable Win32 app dependencies <!-- 2617348 idready eeready  wnready -->
...
#### App version installation information for Microsoft Store for Business apps <!-- 3537391 wnready -->
...
#### Additions to Win32 apps requirement rules <!-- 3676883 wnready -->
You can create requirement rules based on PowerShell scripts, registry values, and file system information. In Intune, select **Client apps** > **Apps** > **Add**. Then select **Windows app (Win32)** as the **App type** in the **Add app** blade.  Select **Requirements** > **Add** to configure additional requirement rules. Then, select either **File type**, **Registry**, or **Script** as the **Requirement type**. For more information, see [Win32 app management](apps-win32-app-management.md).
 ...
 #### Configure your Win32 apps to be installed on Intune enrolled Azure AD joined devices <!-- 3695227 idready eeready wnready -->
...
#### Device overview shows Primary User <!--794259 idready wnready-->
...
#### Additional Managed Google Play app reporting for Android Enterprise work profile devices <!-- 4105925 eeready idready wnready -->
...
#### Set login settings and control restart options on macOS devices <!-- 1210083 eeready idready wnready -->
...
#### Configure WiFi on Android Enterprise, Device Owner dedicated devices running in multi-app kiosk mode <!--3041940 idready wnready -->
You can enable settings on Android Enterprise, Device Owner when running as a dedicated device in multi-app kiosk mode. In this update, you can enable WiFi, and allow users to connect to WiFi networks (**Intune** > **Device configuration** > **Profiles** > **Create profile** > **Android Enterprise** for platform > **Device owner only, Device restrictions** for profile type > **Dedicated devices** > **Kiosk mode**: **Multi-app** > **WiFi configuration**). 
...
#### Configure Bluetooth and pairing on Android Enterprise, Device Owner dedicated devices running in multi-app kiosk mode <!-- 3041941 idready wnready -->
You can enable settings on Android Enterprise, Device Owner when running as a dedicated device in multi-app kiosk mode. In this update, you can enable Bluetooth, and allow users to pair devices over Bluetooth (**Intune** > **Device configuration** > **Profiles** > **Create profile** > **Android Enterprise** for platform > **Device owner only, Device restrictions** for profile type > **Dedicated devices** > **Kiosk mode**: **Multi-app** > **Bluetooth configuration**). 

To see all the settings you can configure, go to [Android Enterprise device settings to allow or restrict features](device-restrictions-android-for-work.md).

Applies to: Android Enterprise dedicated devices running in multi-app kiosk mode

#### Create and use OEMConfig device configuration profiles in Intune <!-- 3305883 eeready idready wnready -->
...
#### Windows Update notifications  <!-- 3316758, 3316782     WNready -->
...
#### New device restriction settings for Android Enterprise, Device Owner <!-- 3574254 eeready idready wnready -->
...
#### Check for a TPM chipset in a Windows 10 device compliance policy <!-- 3617671 idready wnready -->
...
#### Updated UI changes for Microsoft Edge Browser on Windows 10 and later devices <!-- 3775833 wnready -->
...
#### Expanded support for Android Enterprise fully managed devices  (Preview)  <!--   3903241, 3903244, 3903246    WNReady    -->
...
#### Configure profile to skip some screens during Setup Assistant <!-- 2276470 idready wnready wnstaged-->
...
#### Bulk device naming when enrolling corporate iOS devices<!--3566569  idready wnready-->
...
#### Retire noncompliant devices  <!-- 1827291   WNReady  -->
...
#### Scope tags for Apple VPP tokens <!--2371886 idready wnready-->
