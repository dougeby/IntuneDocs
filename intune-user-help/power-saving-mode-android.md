...
The Microsoft Intune team is actively working on a solution for this issue. You can avoid this issue by making sure that your device does not enter low power mode. To stop the issue from happening when you enter low power mode, you must remove the Company Portal and Outlook from the list of apps that are affected by power optimization.
