...
-   **Required install** – The app is installed onto the device, with no end-user intervention required.
    ...
    >
	>  If an end user uninstalls an app that you deployed as a required installation, Intune automatically reinstalls the app after the next inventory cycle, which typically occurs every 7 days.
...
-   If both a required and an available install are received by a device, the actions are combined. In other words, the end user can install the available app from the company portal before the required install begins.
