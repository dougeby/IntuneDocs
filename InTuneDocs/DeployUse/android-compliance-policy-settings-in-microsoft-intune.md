...
- **Require device threat protection to be enabled**: Use this setting to take the risk assessment from the Lookout MTP solution as a condition for compliance. The options for the rule are as follows:
Then select the maximum allowed threat level, which is one of the following:
