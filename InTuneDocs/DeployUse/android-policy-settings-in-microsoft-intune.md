...
|**Number of repeated sign-in failures to allow before the device is wiped**|Specifies the number of failed sign-in attempts before the device is wiped.|Yes|Yes|
|**Minutes of inactivity before screen turns off**|Specifies the number of minutes before the device automatically locks.|Yes|Yes|
...
> [!NOTE]
> A single policy can only contain a list of compliant apps or a list of noncompliant apps. You cannot specify both in the same policy.
