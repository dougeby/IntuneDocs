...
Some managed apps, like the Outlook app for iOS and Android support **multi-identity**. This means that [!INCLUDE[wit_nextref](./includes/wit_nextref_md.md)] only applies management settings to corporate accounts or data in the app.
...
1.  When you are ready to upload the app to your cloud storage space, follow the instructions in [Publish mobile device software to Microsoft Intune cloud storage](deploy-apps-to-mobile-devices-in-microsoft-intune---deleted.md#BKMK_PublishSoftware).
...
2.  Configure and deploy one of the following **Software** policies, depending on the device type you want to configure apps for:
    ...
    You can use recommended settings or customize the settings. For details, see [Use policies to manage computers and mobile devices with Microsoft Intune](use-policies-to-manage-computers-and-mobile-devices-with-microsoft-intune.md).
